...
class Collection extends Base {
    ...
    // Model data returned in the response.
        ...
        if (_.isNil(saved)) {
            ...
            _.each(saved, (data, index) => {
                saving[index].onSaveSuccess(
                    new ProxyResponse(200, data, response.getHeaders())
                );
            });
        }
   ...
   */
    applyValidationErrorObject(errors) {
        let lookup = _.keyBy(this.models, model => model._identifier());
   ...
   */
    onSaveValidationFailure(error) {
        ...
        if (!_.isPlainObject(errors) && !_.isArray(errors)) {
            throw new ResponseError(
                "Validation errors must be an object or array",
                response
            );
        }
   ...
   */
    getIdentifiers(models) {
        return _.map(models, _.method("_identifier"));
    }
